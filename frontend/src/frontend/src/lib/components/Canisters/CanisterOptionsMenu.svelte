<script lang="ts">
    ...
    import { renameCanister, deleteCanister } from '$lib/services/canisterManagement';
...
<Dialog.Root bind:open={renameDialogOpen}>
    <Dialog.Portal>
        ...
        <Dialog.Content class="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 gap-4 border-2 border-[#0B8CE9] bg-[#1F1F1F] p-6 shadow-lg rounded-[21px] font-inder">
            ...
            <div class="space-y-4">
                ...
                <div class="flex justify-end gap-2">
                    <Button variant="outline" on:click={() => renameDialogOpen = false} disabled={isLoading}>
                        Cancel
                    </Button>
                    <Button variant="outline" on:click={handleRename} disabled={isLoading}>
...
<Dialog.Root bind:open={deleteDialogOpen}>
    <Dialog.Portal>
        ...
        <Dialog.Content class="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 gap-4 border-2 border-[#0B8CE9] bg-[#1F1F1F] p-6 shadow-lg rounded-[21px] font-inder">
            ...
            <p class="mb-4">Are you sure you want to delete "{canisterName}"? This action cannot be undone.</p>
            {#if error}
                <p class="text-red-500 text-sm mb-4">{error}</p>
            {/if}
            <div class="flex justify-end gap-2">
                <Button variant="outline" on:click={() => deleteDialogOpen = false} disabled={isLoading}>
                    Cancel
                </Button>
                <Button variant="outline" class="text-red-500" on:click={handleDelete} disabled={isLoading}>
